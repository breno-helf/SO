...
WorstFit::WorstFit(BiFile &virMem, int total, int virt, int s, int p) {
    file = &virMem;
    this->total = total;
    this->virt = virt;
    this->s = s;
...
bool WorstFit::insert(char pid, int b) {
    int pidSize = (b/s);
    int size = (b/pSize);
    if (b%s) pidSize++;
    if (b%pSize) size++;
...
void WorstFit::printll() {
    l->print();
}
        
void WorstFit::compact(int *pageMap) {
...
int WorstFit::translate(char pid, int p) {
    node *curr;
    curr = l->head;
    while (curr != NULL && curr->pid != pid)
        curr = curr->next;
